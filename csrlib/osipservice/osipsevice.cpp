...
#pragma comment(lib, "win32_2.lib")
...
int init_udpsocket(int port, struct sockaddr_in *servaddr, char *mcast_addr) {
	...
	char* loop = "1";
	err = setsockopt(socket_fd, IPPROTO_IP, 11, loop, sizeof(loop));
	if (err < 0) {
...
static int startStreamRecv(liveVideoStreamParams *pliveVideoParams) {
	...
	for (i = 0; i < pliveVideoParams->cameraNum; i++) {
		...
		p->statusErrCnt = 0;
		p->running = 1;
		if ((hRtpHandle = (HANDLE)_beginthreadex(NULL, 0, rtp_recv_thread, (void*)p, 0, NULL)) != INVALID_HANDLE_VALUE) {
			APP_ERR("pthread_create rtp_recv_thread err, %s:%d", p->sipId, p->recvPort);
		}
		else {
			CloseHandle(hRtpHandle);
		}
		if ((hAliveHandle = (HANDLE)_beginthreadex(NULL, 0, stream_keep_alive_thread, (void*)p, 0, NULL)) != INVALID_HANDLE_VALUE) {
			APP_ERR("pthread_create stream_keep_alive_thread err, %s:%d", p->sipId, p->recvPort + 1);
		}
		else {
			CloseHandle(hAliveHandle);
		}
	}
